...
公共内容只在 core 维护。安装命令自动构建 dist/claude-code/solo，再安装完整真实文件副本到 ~/.claude/skills/solo；不依赖内部资源软链接。修改共享源后再次执行同一命令更新，已手工改动的安装文件会报告冲突并保留。

安装结构可通过回归测试；本次未检测到 Claude Code 命令行程序，选择器及端到端执行尚未验证。

...
快照版本：1.4。基线来源：用户确认的 ddd 参考工程，提交 `dad0bd8`，并同步 SRC-040 的纯注入构造器注释清理/门禁诊断及本轮 SRC-041 的 21 个注入依赖字段注释移除/字段门禁豁免（这些调整尚未提交）。源码、POM、资源、测试与根 Checkstyle 与当前参考工程对应文件一致；Java 规范版本为 1.5。本说明与 .gitignore 为快照补充文件。

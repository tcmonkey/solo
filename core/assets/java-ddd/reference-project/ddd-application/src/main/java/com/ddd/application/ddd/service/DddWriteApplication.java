 ...
 * DDD 写模式的应用服务模板。
 *
 * <p>仅将应用命令组装为包含输入聚合的领域参数， 再委托领域服务完成加载、实体确认和保存。
 *
 * @author AIGenerator
 */
@Service
public class DddWriteApplication {
     ...
     * 将应用命令委托给领域写服务处理。
     *
     * <p>本层只传递原始命令数据；聚合及其根实体负责封装值对象和子操作实体。
     *
            ...
            return new org.springframework.transaction.support.TransactionTemplate(
                    ...
                    .execute(
                            status -> {
                                ...
                                // 2. 调用领域服务完成写入决策。
                                Result<DddWriteDO> domainResult =
                                        dddWriteDomainService.write(param);
                                if (!domainResult.success()) {
                                    status.setRollbackOnly();
                                    return Result.failure(
                                            domainResult.code(), domainResult.message());
                                }
                                // 3. 将领域内部数据对象转换为应用层结果。
                                DddWriteDO dataObject = domainResult.data();
                                DddWriteResult result =
                                        assembler.toResult(dddWriteCommand, dataObject);

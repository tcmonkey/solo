...
入口、阶段规范和宿主使用说明统一采用中文。`SKILL.md` 等协议文件名、配置键、机器状态值、命令和代码标识符保持原样，便于脚本及各宿主识别；语言调整不改变工作流程或宿主安装方式。
...
| 模式 | 适用情况 | 默认阶段 |
|---|---|---|
| Lite | Bug、脚本、小改动 | 需求 → 技术 → 开发与自测 → CR → 独立测试 |
| Standard | 常规产品功能 | 需求 → 产品 → 可选 UI → 技术 → 开发与自测 → CR → 独立测试 → 发布检查 |
| Full | 新产品、跨系统或高风险功能 | 全流程，增加 UI、安全、迁移、按需性能测试、发布和复盘要求 |

`guided` 在关键语义关口等待确认；`continuous` 在授权范围内持续执行到指定阶段。
...
已有 `0.1.x`～`0.6.x` 项目第一次使用 `0.7.0` 时执行：
...
迁移会备份旧状态、项目配置和被改名文档，把输入材料清单迁移为 `AI/input/00 输入材料清单.md`，把人类可读产物迁移到 `AI/output` 并统一使用带阶段序号的中文文件名。对于 `0.1.x`～`0.3.x`，还会把 `implementation` 迁移为 `development`，并把合并的 `verification` 拆成 `code_review` 与 `testing`。

## 开发、CR 与测试的边界

- **开发与自测**：编码，以及编译/类型检查、Lint、单元测试、开发侧集成检查和最小冒烟；产物为 `06 开发自测报告.md`。
- **代码审查**：检查实际 diff 的正确性、安全、数据、并发、兼容、可维护性与开发测试充分性；产物为 `07 代码审查报告.md`。
- **独立测试**：从需求和产品视角设计用例并执行功能、接口/E2E、权限、边界、异常、回归等测试；产物为 `08 测试用例.md`、`09 测试报告.md` 和 `10 缺陷记录.md`。
- **性能测试**：只有存在明确非功能指标、发布标准或风险时才启动，单独形成 `11 性能测试报告.md`，不作为所有小项目的固定动作。

## Java DDD 编码规范与自动检查

采用本工具 Java DDD 参考规范时，先阅读 [Java DDD开发规范](<core/references/Java DDD开发规范.md>)。开发阶段会为新 Maven 聚合工程安装根 `checkstyle.xml`、根 POM 的 validate 门禁和 `AI/output/19 Java DDD开发规范.md` 快照；无需逐个子模块配置。已有检查配置冲突时先融合，不直接覆盖。
...
项目经验先记录到该项目的 `AI/output/13 项目复盘.md`。确认应成为公共规则后，再修改 `solo/core/` 并重新构建发行包。不要让单个项目自动改写公共模板。
